...
function apply(ctx, config) {
    ...
    async function handleSTMessage(msg) {
        switch (msg.type) {
            ...
            case 'send_message_result':
                if (!msg.success)
                    stopTypingTimer(msg.sourceChannelKey);
    ...
    async function sendTyping(session) {
        try {
    ...
    function startTypingTimer(session) {
    ...
    function stopTypingTimersForChat(chatId) {
    ...
    async function setTypingStatus(chatId, typing) {
        if (typing) {
