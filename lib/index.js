...
function apply(ctx, config) {
    ...
    async function sendTemporary(platform, channelId, content, deleteAfterMs = 30000) {
        const bot = findBot(platform);
        if (!bot)
            return;
        try {
    ...
    async function broadcastTemporary(content, deleteAfterMs = 30000, platformFilter) {
        const query = {};
        if (platformFilter)
            query.platform = platformFilter;
        const bindings = await ctx.database.get('st_bindings', query);
                ...
                for (const el of session.elements) {
                    ...
                    switch (el.type) {
                        ...
                        default: {
                            ...
                            if (!data && session.platform === 'telegram' && !session.event?._data?.mediaGroup) {
