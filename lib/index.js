            ...
            try {
                ...
                if (hasImages && binding.platform === 'telegram') {
                    ...
                    for (let i = 0; i < msg.content.images.length; i++) {
                        ...
                        await bot.internal.sendPhoto(formData);
                    }
                }
                else if (hasImages) {
        ...
        for (const binding of bindings) {
            ...
            try {
                if (binding.platform === 'telegram') {
                    ...
                    await bot.internal.sendVoice(formData);
                }
                else {
    ...
    ctx.middleware(async (session, next) => {
        return next(async (done) => {
            ...
            if (hasAudio) {
                ...
                logger.info(`session.event._data keys: ${JSON.stringify(Object.keys(session.event?._data || {}))}`);
                logger.info(`session.event._data: ${JSON.stringify(session.event?._data).substring(0, 500)}`);
            }
            if (session.elements) {
                    ...
                    switch (el.type) {
                        ...
                        case 'record': {
                            ...
                            if (src && (src.startsWith('http://') || src.startsWith('https://'))) {
                                ...
                                const rawEvent = session.event?._data || session.event?.telegram || session.event;
