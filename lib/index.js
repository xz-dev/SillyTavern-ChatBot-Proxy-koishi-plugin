...
function apply(ctx, config) {
     ...
     */
    async function sendStatusNotification(category, platform, channelId, text, alsoDeleteCategories) {
        const bot = findBot(platform);
        if (!bot)
            return;
     ...
     */
    async function broadcastStatusNotification(category, text, platformFilter, alsoDeleteCategories) {
        const query = {};
        if (platformFilter)
            query.platform = platformFilter;
        const bindings = await ctx.database.get('st_bindings', query);
