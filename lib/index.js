...
function apply(ctx, config) {
    ...
    ctx.on('dispose', () => {
        ...
        for (const timer of botOfflineTimers.values())
            clearTimeout(timer);
    ...
    ctx.on('login-updated', (session) => {
        ...
        else if (bot.status === 0 /* Universal.Status.OFFLINE */ || bot.status === 3 /* Universal.Status.DISCONNECT */) {
        ...
        else if (bot.status === 4 /* Universal.Status.RECONNECT */) {
    ...
    // ----------------------------------------------------------
    async function recoverMissedMessages(bot) {
        if (!activeClient || activeClient.readyState !== 1) {
        ...
        for (const binding of bindings) {
            ...
            if (!lastId)
                continue; // Never processed a message here — skip
            try {
                const missed = [];
                for await (const msg of bot.getMessageIter(binding.channelId)) {
                    ...
                    if (msg.user?.id === bot.selfId)
                        continue;
                    // Safety limit
                    if (missed.length >= MAX_RECOVERY_MESSAGES)
                        break;
                    const sender = msg.user?.name || msg.user?.nick || msg.user?.id || 'Unknown';
                    const content = msg.content || msg.elements?.map((e) => {
                        if (e.type === 'text')
                            return e.attrs?.content || '';
                        if (e.type === 'img' || e.type === 'image')
                            return '[Image]';
                        if (e.type === 'audio' || e.type === 'voice' || e.type === 'record')
                            return '[Audio]';
                        if (e.type === 'video')
                            return '[Video]';
                        if (e.type === 'file')
    ...
    function findBot(platform) {
        return ctx.bots.find(b => b.platform === platform && b.status === 1 /* Universal.Status.ONLINE */);
    }
     ...
     */
    async function sendAiMessageWithReasoning(platform, channelId, bot, nameHeader, reasoning, bodyText) {
        try {
            switch (platform) {
                case 'telegram': {
                    const parts = [];
                    if (nameHeader)
                        parts.push(`<blockquote>${escapeHtml(nameHeader)}</blockquote>`);
     ...
     */
    async function sendStatusNotification(category, platform, channelId, text, alsoDeleteCategories, autoDeleteMs) {
        const bot = findBot(platform);
        if (!bot)
            return;
     ...
     */
    async function broadcastStatusNotification(category, text, platformFilter, alsoDeleteCategories, autoDeleteMs) {
        const query = {};
        if (platformFilter)
            query.platform = platformFilter;
        const bindings = await ctx.database.get('st_bindings', query);
        ...
        for (const binding of bindings) {
            ...
            try {
                ...
                if (reasoning && !hasImages) {
                    ...
                    const sentIds = await sendAiMessageWithReasoning(binding.platform, binding.channelId, bot, nameHeader, reasoning, bodyText);
